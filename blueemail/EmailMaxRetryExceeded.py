...
class EmailMaxRetryExceeded(BaseEmail):
    ...
    def get_message(self) -> HtmlMessage:
        ...
        text = (
                ...
                f'<p>Zapis na zajęcja <b>{self.activity_to_book.name}</b> odbywające się <b>{Weekday.name_pl_acc(self.actactivity_to_bookivity.weekday)}</b> o godz. <b>{self.activity_to_book.hour}</b> w klubie <b>{self.activity_to_book.club.get_name()}</b> nie powiódł się.<br/>'

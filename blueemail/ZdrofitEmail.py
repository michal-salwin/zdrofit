...
class ZdrofitEmail:
    ...
    def send_on_activity_not_found(self, activity: Activity):
        ...
        text = (
                ...
                f'<p>{self.user.get_first_name_vocative()},</p>'
                f'<p>Zapis na zajęcja <b>{activity.name}</b> odbywające się <b>{Weekday.name_pl_acc(activity.weekday)}</b> o godz. <b>{activity.hour}</b> w klubie <b>{activity.club.get_name()}</b> nie powiódł się.</p>'
    ...
    def send_on_max_retry_exceeded(self, activity: Activity, nr_of_retries: int):
        ...
        text = (
                ...
                '<h3>Zdrofit automatyczna rejestracja - problemik :(</h3>'
                f'<p>{self.user.get_first_name_vocative()},</p>'
                f'<p>Zapis na zajęcja <b>{activity.name}</b> odbywające się <b>{Weekday.name_pl_acc(activity.weekday)}</b> o godz. <b>{activity.hour}</b> w klubie <b>{activity.club.get_name()}</b> nie powiódł się.<br/>'
